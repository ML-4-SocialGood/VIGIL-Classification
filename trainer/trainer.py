...
class Trainer:
    ...
    def train(self):
        ...
        for self.current_epoch in range(self.max_epoch):
            self.before_epoch()
            batch_sigma = self.run_epoch()
            batch_sigma = np.array(batch_sigma)
            epoch_sigma_1.append(np.mean(batch_sigma[:, 0]))
            epoch_sigma_2.append(np.mean(batch_sigma[:, 1]))
            print(epoch_sigma_1)
            print(epoch_sigma_2)
            # exit()
            self.after_epoch()
    ...
    def run_epoch(self):
        ...
        for self.batch_idx, batch_data in enumerate(self.data_loader_train):
            ...
            loss_summary, sigma = self.forward_backward(batch_data)
            sigmas.append(sigma)
